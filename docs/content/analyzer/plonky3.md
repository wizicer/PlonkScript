You can try the [analyzer](/play/index.html#/analyzer/plonky3){target="_blank"} now.

To start analysis, inject the code like this to get the trace for analysis:

```rust
plonky3_summarizer::save_as_json::<F, _>(&chip, &trace, &pis, "file.json")?; // [!code ++]
```
## Using package:
...
```toml
[dependencies]
plonky3_summarizer = "0.0.1"
```


## Inject whole source code: 

```rust{96-101}
mod plonky3_summarizer {
    use std::fs::File;
    use std::io::Write;
